...
Box2DFactory().then(box2D => {
    ...
    const createChainShape = function (vertices, closedLoop = true) {
      ...
      const verts = [
        new b2Vec2(-40+40, 0+40),
                new b2Vec2(-20+40, 20+40),
                new b2Vec2(0+40, 0+40),
                new b2Vec2(20+40, -20+40),
                new b2Vec2(40+40, 0+40),
                new b2Vec2(-40+40, 0+40)
      ];
    ...
    //   verts.reverse();
      const chainShape = createChainShape(verts);//true for closed loop *** some problem with this atm
    ...
    const gravity = new b2Vec2(0, 10); // 重力向下
...
body.CreateFixture(chainShape, 0.0);
...
console.log(fixtureDef);
   ...
   * @returns {Box2D.b2Vec2}
   */
    const scaledVec2 = (vec, scale) => new b2Vec2(
        scale * vec.get_x(),
        scale * vec.get_y()
    );
   ...
   * @return {Box2D.b2Vec2[]}
   */
    const reifyArray = (array_p, numElements, sizeOfElement, ctor) => Array(
        numElements
    )
        .fill(undefined)
